...
def test_create_user(mock_user_create, client, user):
    ...
    res = client.post('/api/user/', data=json.dumps(user), content_type='application/json')
...
def test_get_user(mock_user_get, client, user):
    ...
    res = client.get(f'/api/user/{mock_user.id}')
    assert res.status_code == 200
...
def test_update_user(mock_where, mock_user_update, client, user):
    ...
    res = client.put(f'/api/user/{updated_user.id}', data=json.dumps(user), content_type='application/json')
...
def test_delete_user(mock_where, mock_user_delete, client, user):
    ...
    # Make a request to the API endpoint
    res = client.delete(f'/api/user/{delete_user.id}', data=json.dumps(user), content_type='application/json')
